...
namespace aue {
    ...
    AnswerCodes WorldMap::addObject(WObject object) {
        ...
        map2D.insert({object->get_pos(), object});
        return AnswerCodes::OK;
    }

    std::vector<WObject> WorldMap::getObjectsInSquare(aue::pos_t left_down_point, aue::pos_t right_up_point) const {
        auto left_iter = map2D.upper_bound(left_down_point);
        auto right_iter = map2D.lower_bound(right_up_point);
        std::vector<WObject> result;
        do {
